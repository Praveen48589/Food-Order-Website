# Deployment Playbook —  Food Order website on EKS
...
```bash
aws eks update-kubeconfig --name foodapp-eks --region ap-south-1
kubectl get nodes
...
- `foodapp.praveentomar.com` → `<NLB hostname from Step 4>`

```bash
# Check certificate status
kubectl get certificate -n foodapp
kubectl get secret bankapp-tls -n foodapp
```
...
docker buildx build --platform linux/amd64 \
  -t praveen416/foodapp-eks:latest \
  --push .
```
...
# Watch sync progress
kubectl get application foodapp -n argocd -w
```
...
kubectl get gateway -n foodapp

# Get the app URL (NLB created by Envoy Gateway)
kubectl get svc -n envoy-gateway-system \
  -l gateway.envoyproxy.io/owning-gateway-name=foodapp-gateway \
...
|---------|------------|-------------|
| **foodApp** | `kubectl get svc -n envoy-gateway-system -l gateway.envoyproxy.io/owning-gateway-name=foodapp-gateway -o jsonpath='{.items[0].status.loadBalancer.ingress[0].hostname}'` | App login |
